...
@interface HSWalletTodayVC ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation HSWalletTodayVC
...
- (void)viewDidLoad {
    ...
    self.headerImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, kTopHeight, kScreenWidth, kRealValue(183))];
     ...
     self.headerImageView.image = kGetImage(@"wallet_today_head");
    ...
    NSMutableAttributedString *hotStr = [[NSMutableAttributedString alloc]      initWithString:@"今日收入（火币）"];
    ...
    [mxBtn setTitleColor:[UIColor colorWithHexString:@"#E2BD81"] forState:0];
    ...
    [[MHUserService sharedInstance]initwithHSDayShouruCompletionBlock:^(NSDictionary *response, NSError *error) {
...
- (UIView *)naviView {
    if (!_naviView) {
        ...
        _naviView.image = kGetImage(@"wallet_today_nav");
