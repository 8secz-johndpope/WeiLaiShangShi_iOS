...
@interface MHPickView()<UIPickerViewDelegate,UIPickerViewDataSource>
...
-(void)setupUI{
    ...
    _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(125, 0, kRealValue(100), 44.0/667.0*screenH)];
...
-(void)sureClick{
    if (self.sureBlock) {
        if (ValidStr(self.oneStr)) {
            self.sureBlock(self.oneStr,self.selectIndex);
        }else{
            self.sureBlock(@"中国工商银行",0);
        }
    }
    [self removeFromSuperview];
}
