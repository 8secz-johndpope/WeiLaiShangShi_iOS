...
@implementation HSNewsOneLeftCell
...
-(void)createviewWithModel:(HSNewsModel *)createmodel
{
    ...
    for (int i=0; i<createmodel.cover.count; i++) {
        NSString *str =[createmodel.cover objectAtIndex:i];
        NSString *Str1 = [str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        //        [self.image sd_setImageWithURL:[createmodel.cover objectAtIndex:i] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ////             NSLog(@"错误信息:%@",error);
        //        }];
        [self.image sd_setImageWithURL:[createmodel.cover objectAtIndex:0] placeholderImage:kGetImage(@"emty_movie")];
    ...
    if (createmodel.top == 1) {
        ...
        [self.Toplabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.image.mas_left).offset(kRealValue(0));
            make.top.equalTo(self.image.mas_bottom).offset(kRealValue(10));
...
-(void)createview
{
    ...
    [self.image mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.titlelabel.mas_left).offset(kRealValue(-12));
